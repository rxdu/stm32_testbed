...
uint8_t tx_buffer[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77};
...
void TestbedUartTask(void* param) {
  ...
  for (int i = 0; i < UART_CHN_NUM && uart_cfg.channel[i].handler != NULL;
       ++i) {
  ...
  for (;;) {
    for (int i = 0; i < UART_CHN_NUM && uart_cfg.channel[i].handler != NULL;
      ...
      if (xSemaphoreTake(uart_cfg.channel[i].rx_semaphore, UART_RX_TIMEOUT) ==
        ...
        for (int j = 0; j < 8; ++j) {
          if (uart_cfg.channel[0].rx_buffer[i] != tx_buffer[i]) {
            echo_matched = false;
            break;
          }
        }
        ...
        DPrintf(
            0,
            "[INFO][UART] Echo not received within TIMEOUT (%d ms) from %s\n",
